...
class McpProductService
{
    ...
    public function upsert(string $slug, array $data, int $tokenId, bool $dryRun): array
    {
        ...
        try {
            DB::transaction(function () use ($slug, $data, $tokenId, $dryRun, &$preview, &$autoCreated, &$protectedFieldsSkipped) {
                ...
                // 4b. Filter values (facet/variant-dimension assignment) — additive only.
                // Unknown slugs are silently skipped (whereIn just won't match them) rather
                // than aborting the whole product save over one typo.
                $touchedFilterValues = false;
                if (! empty($data['filter_values'])) {
                    $ids = FilterValue::whereIn('slug', $data['filter_values'])->pluck('id');
                    $product->filterValues()->syncWithoutDetaching($ids);
                    $touchedFilterValues = true;
                }
                if (! empty($data['remove_filter_values'])) {
                    $ids = FilterValue::whereIn('slug', $data['remove_filter_values'])->pluck('id');
                    $product->filterValues()->detach($ids);
                    $touchedFilterValues = true;
                }
                if ($touchedFilterValues) {
                    // Pivot writes don't touch the Product row, so Scout's Searchable
                    // trait never auto-reindexes them — same reason Filament's own
                    // ManagesProductRelations::syncSearchIndex() needs this explicitly.
                    $product->searchable();
                }
                ...
                $product->refresh()->load(['brand', 'manufacturer', 'categories.translations', 'translations', 'seoMetas', 'attributes', 'geoProfiles', 'filterValues']);
    ...
    private function computeReadiness(Product $product): array
    {
        ...
        if (! $hasSku) {
            $blocking[] = 'general.has_sku — SKU chưa được set';
        }
        if (! $hasPrice) {
            $blocking[] = 'general.has_price — price = 0 hoặc chưa được set';
        }
        if (! $hasCategory) {
            $blocking[] = 'general.has_category — product chưa có danh mục';
        }
        if (! $hasManufacturer) {
            $blocking[] = 'general.has_manufacturer — product chưa có nhà sản xuất';
        }
    ...
    private function buildContextResponse(Product $product): array
    {
        ...
        return [
            ...
            'filter_values' => $product->filterValues->pluck('slug')->values()->all(),
    ...
    private function loadProduct(string $slug): Product
    {
        $product = Product::where('slug', $slug)
            ->with(['brand', 'manufacturer', 'categories.translations', 'translations', 'seoMetas', 'attributes', 'geoProfiles', 'filterValues'])
            ->first();
