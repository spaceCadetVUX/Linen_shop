...
class McpManufacturerService
{
    ...
    public function upsert(string $slug, array $data, int $tokenId, bool $dryRun): array
    {
        ...
        try {
            DB::transaction(function () use ($slug, $data, $tokenId, $dryRun, &$preview, &$protectedFieldsSkipped) {
                ...
                // ── SEO meta ──────────────────────────────────────────────────
                if (! empty($data['seo'])) {
                    $this->writeSeoMeta($mfr, $data['seo'], $overwrite, $protectedFieldsSkipped);
                }
    ...
    private function writeSeoMeta(Manufacturer $mfr, array $seo, bool $overwrite, array &$skipped = []): void
    {
